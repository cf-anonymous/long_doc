...
from run_classifier_dataset_utils_lstm import (compute_metrics, gpt2_convert_examples_to_features,
...
class GPT2ClassificationModel(GPT2PreTrainedModel):
    ...
    def forward(self, input_ids, labels=None):
        hidden_states = self.transformer(input_ids[:,0])[0]
        cls_rep = hidden_states[:,-1]
        cls_rep = self.dropout(cls_rep)

        logits = self.classifier1(cls_rep)
...
class NewAttention(torch.nn.Module):
    def __init__(self, sent_hidden_size=784):
        ...
        self.rnn = torch.nn.LSTM(sent_hidden_size, sent_hidden_size, batch_first=True , bidirectional=True)
        self.word_attention = torch.nn.Linear(2 * sent_hidden_size, sent_hidden_size)

        # Word context vector to take dot-product with
        self.word_context_vector = torch.nn.Linear(sent_hidden_size, 1, bias=False)
...
class AttentionLongGPT2(GPT2PreTrainedModel):
    ...
    def forward(self, input_ids, labels=None):
        ...
        for i in range(input_ids.size(1)):
            hidden_states = self.transformer(input_ids[:,i])[0].detach()
            z = hidden_states[:,-1]
            zs.append(z)
        ...
        attention_z = self.transformer(torch.stack(att_id))[0][:,-1]
        ...
        first_z = self.transformer(input_ids[:,0])[0][:,-1]
...
class LongGPT2(GPT2PreTrainedModel):
    """BERT model for classification.
    This module is composed of the BERT model with a linear layer on top of
    the pooled output.
    """
    def __init__(self, config):
        super(LongGPT2, self).__init__(config)
        self.sequence_len = config.sequence_len
        self.transformer = GPT2Model(config)
        
        self.dropout = torch.nn.Dropout(config.summary_first_dropout)
        self.lstm = torch.nn.LSTM( config.n_embd,  config.n_embd * 2, bidirectional = True)
        self.classifier1 = torch.nn.Linear(config.n_embd * 9, config.num_labels)
...
class AblationLongGPT2(GPT2PreTrainedModel):
    """BERT model for classification.
    This module is composed of the BERT model with a linear layer on top of
    the pooled output.
    """
    ...
    def forward(self, input_ids, labels=None):
        ...
        zs = []
        for i in range(input_ids.size(1)):
            hidden_states = self.transformer(input_ids[:,i])[0]
            if i > 0:
...
MODELS_DICT = {
    "base": GPT2ClassificationModel,
    "long": LongGPT2,
    "ablation": AblationLongGPT2,
    "attention": AttentionLongGPT2,
    }
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            model.train()
            batch[0] = batch[0].view(-1, args.seq_segments, args.max_seq_length)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file):
        ...
        features = gpt2_convert_examples_to_features(examples, label_list, args.max_seq_length, args.seq_segments, tokenizer, output_mode)
...
def main():
    ...
    parser.add_argument("--seq_segments", default=8, type=int,
                        help="Number of sequences to input for the rnn")
    ...
    parser.add_argument("--experiment", default="attention", type=str, required=False,
                        ...
                        "base: original gpt2"
                        "long: uses an lstm to keep track of all hidden representations, but backprop over the first"
    ...
    model_class = MODELS_DICT[args.experiment]
    tokenizer_class = GPT2Tokenizer
    config = config_class.from_pretrained('gpt2', num_labels=num_labels, finetuning_task=args.task_name)#args.config_name if args.config_name else args.model_name_or_path, num_labels=num_labels, finetuning_task=args.task_name)
    tokenizer = tokenizer_class.from_pretrained('gpt2')#args.tokenizer_name if args.tokenizer_name else args.model_name_or_path, do_lower_case=args.do_lower_case)
    
    config.sequence_len = args.seq_segments
    model = model_class.from_pretrained('gpt2', from_tf=False, config=config)
