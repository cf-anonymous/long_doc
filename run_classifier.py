...
from run_classifier_dataset_utils import processors, output_modes, convert_examples_to_features, compute_metrics, parallel_convert_examples_to_features
...
def main():
    ...
    if args.do_train:
        ...
        #class_weight = calculate_class_weights([f.label_id for f in train_features])
        ...
        model.train()
        weights = torch.FloatTensor([1] * num_labels).cuda()
        
        #weights = torch.FloatTensor([.3,1]).cuda()
        for i in trange(int(args.num_train_epochs), desc="Epoch", disable=args.local_rank not in [-1, 0]):
            if i > 0 and args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0): #do eval epoch
                model.eval()
                eval_loss = 0
                nb_eval_steps = 0
                preds = []
                ...
                for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
                    ...
                    if output_mode == "classification":
                        ...
                        tmp_eval_loss = loss_fct(logits.view(-1, num_labels), label_ids)
                ...
                output_eval_file = os.path.join(args.output_dir, "eval_results{}.txt".format(i))
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0])):
                ...
                if output_mode == "classification":
                    ...
                    loss_fct = BCEWithLogitsLoss()
                    loss = loss_fct(logits.view(-1, num_labels), label_ids)
